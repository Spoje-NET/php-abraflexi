    ...
    public static function makeTestPayment($initialData = [], $dayBack = 1)
    {
        ...
        if ($payment->sync()) {
            $payment->addStatusMessage(
                $payment->getApiURL().' '.\AbraFlexi\RO::uncode($payment->getDataValue('typPohybuK')).' '.\AbraFlexi\RO::uncode($payment->getRecordIdent()).' '.\AbraFlexi\RO::uncode($payment->getDataValue('sumCelkem')).' '.\AbraFlexi\RO::uncode($payment->getDataValue('mena')),
                'success',
            );
        } else {
