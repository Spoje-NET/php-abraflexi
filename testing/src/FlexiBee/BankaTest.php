...
class BankaTest extends FlexiBeeRWTest
{
    ...
    public static function makeTestPayment($initialData = [], $dayBack = 1)
    {
        ...
        $payment = new \FlexiBee\Banka($initialData);

        $payment->takeData(array_merge([
            'kod' => $testCode,
            'banka' => 'code:HLAVNI',
            'typPohybuK' => 'typPohybu.prijem',
            'popis' => 'FlexiBee Test bank record',
            ...
            'typDokl' => \FlexiBee\RO::code('STANDARD')
                ], $initialData));
        if ($payment->sync()) {
            $payment->addStatusMessage($payment->getApiURL().' '.\FlexiBee\RO::uncode($payment->getDataValue('typPohybuK')).' '.\FlexiBee\RO::uncode($payment->getRecordIdent()).' '.\FlexiBee\RO::uncode($payment->getDataValue('sumCelkem')).' '.\FlexiBee\RO::uncode($payment->getDataValue('mena')),
                'success');
        } else {
    ...
    public function getDataForInsert($code = 'UnitTest')
    {
        ...
        $bankCodeRaw                 = $loader->getColumnsFromFlexibee([
            'kod'], ['limit' => 1]);
        $dataForInsert['banka']      = \FlexiBee\RO::code($bankCodeRaw[0]['kod']);
        $dataForInsert['typPohybuK'] = 'typPohybu.prijem';
        return $dataForInsert;
    }

    /**
     * @covers FlexiBee\Banka::stahnoutVypisyOnline
     */
    ...
    /**
     * @covers FlexiBee\Banka::automatickeParovani
     */
