...
function getEvidenceFormats($evidence, FlexiBeeRO $syncer) {
    ...
    $syncer->setFormat('json');
    $flexinfo = $syncer->getColumnsFromFlexibee(['id'], ['limit' => 1]);
    if (!empty($flexinfo) && is_array($flexinfo) && is_array($flexinfo[0]) && array_key_exists('id', $flexinfo[0])) {
        $id = is_numeric($flexinfo[0]['id']) ? intval($flexinfo[0]['id']) : $flexinfo[0]['id'];
        $formats = [];
        $sensibleFormats = Formats::$formats;
        unset($sensibleFormats['TTF']);
        unset($sensibleFormats['TEXT']);
        unset($sensibleFormats['JS']);
        unset($sensibleFormats['PNG']);
        unset($sensibleFormats['CSS']);
        foreach ($sensibleFormats as $cancode => $candidate) {
...
$syncer = new FlexiBeeRO(null, ['ignore404' => true]);
